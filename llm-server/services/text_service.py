...
class TextModelService(BaseModelService):
    ...
    def _format_prompt(self, messages: List[Dict[str, str]]) -> str:
        """Format messages according to DeepSeek-R1 requirements"""
        formatted_messages = []
        for msg in messages:
    ...
    def _get_cache_key(self, messages: List[Dict[str, str]], temperature: float) -> str:
        ...
        message_str = json.dumps([{"role": m["role"], "content": m["content"]} for m in messages], sort_keys=True)
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            test_cmd = ["/home/azureuser/llama.cpp/main", "-m", str(self.model_path), "--n-gpu-layers", "1", "--ctx-size", "8", "--prompt", "<｜User｜>test<｜Assistant｜>", "-n", "1"]

            process = await asyncio.create_subprocess_exec(*test_cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
        ...
        try:
            ...
            async with self.model_lock:
                ...
                cmd = [
                    ...
                    str(self.model_path),
                    "--ctx-size",
                    "163840",  # Maximum context size
                    "--batch-size",
                    "4096",  # Optimized for H100
                    "--threads",
                    "40",  # Match vCPU count
                    "--temp",
                    str(temperature),
                    "--n-gpu-layers",
                    "40",  # Use H100 efficiently
                    "--mlock",  # Keep in memory
                    "--numa",  # NUMA optimization
                    "--rope-scaling",
                    "yarn",  # Better position encoding
                    "-p",
                    prompt,
                ]

                if request.get("max_tokens"):
                    cmd.extend(["-n", str(request["max_tokens"])])
